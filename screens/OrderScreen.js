...
//               setSelectedOrders([]);
//             } catch (error) {
//               Alert.alert('Error', 'Failed to cancel orders');
//             }
//           }
//         }
//       ]
...
//           ...(selectedPayment === 'Cash' && { cash_amount: parseFloat(cashAmount) })
//         })))
...
//       setShowPaymentModal(false);
//       setCashAmount('');
//       Alert.alert('Success', 'Orders settled successfully');
//     } catch (error) {
...
//           {item.customer?.phone && (
//             <Text style={styles.customerPhone}>
//               {item.customer?.country_code || '+971'} {item.customer.phone}
//             </Text>
//           )}
//         </View>
//         <View style={styles.orderMeta}>
//           <Text style={styles.orderTime}>{formatTime(item.created_at)}</Text>
//           {item.table && <Text style={styles.orderTable}>{item.table.name}</Text>}
//           <Text style={styles.orderType}>{item.order_type?.replace('_', ' ')}</Text>
//         </View>
//       </View>

//       <View style={styles.orderDetails}>
//         <Text style={styles.itemsCount}>
//           {item.items?.length || 0} items • QAR {item.total_price?.toFixed(2) || '0.00'}
//         </Text>

//         {item.items?.map((orderItem, index) => (
//           <View key={index} style={styles.orderItem}>
//             <Text style={styles.itemName}>{orderItem.menu.name}</Text>
//             <Text style={styles.itemPrice}>
//               {orderItem.quantity} x {orderItem.price.toFixed(2)} = QAR {(orderItem.quantity * orderItem.price).toFixed(2)}
//             </Text>
//           </View>
//         ))}
//       </View>
